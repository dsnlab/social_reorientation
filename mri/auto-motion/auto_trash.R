...
#------------------------------------------------------
# paths
rpDir = '/Volumes/psych-cog/dsnlab/SFIC_Self3/notes/motion/rp_txt/'
outputDir = '/Volumes/psych-cog/dsnlab/SFIC_Self3/notes/motion/auto-motion-output/'
rpOutputDir = '/Volumes/psych-cog/dsnlab/SFIC_Self3/notes/motion/rp_auto_txt/'
plotDir = '/Volumes/psych-cog/dsnlab/SFIC_Self3/notes/motion/plots/'

# variables
study = "SFIC"
rpPattern = "^rp_(s[0-9]{3})_(.*).txt"
...
# modify volume numbers for s004 (t1, 100) and s095 (t1, 129) --> missing one volume each
intensities = intensities %>% mutate(volume = ifelse(subjectID == "s004" & run == "t1" & volume > 99, volume-1, volume),
                                      volume =ifelse(subjectID == "s095" & run == "t1" & volume > 128, volume-1, volume))
...
trash = intensities %>% 
  ...
  mutate(meanDiff.mean = mean(Diff.mean, na.rm=TRUE),
         ...
         # code volumes with more than +/- 1mm translation in Euclidian distance
...
#------------------------------------------------------
# load manually coded data and merge with trash
#------------------------------------------------------
...
joined = trash %>% 
  left_join(., manual) %>%
  mutate(trash.joined = ifelse(trash.combined == 1 | trash == 1, 1, 0))
...
summary.run = joined %>% 
  group_by(subjectID, run) %>% 
  summarise(nVols = sum(trash.joined, na.rm = T),
            percent = round((sum(trash.joined, na.rm = T) / n())* 100,1))
...
summary.trash = joined %>%
  filter(trash.combined == 1) %>%
  select(subjectID, run, volume, trash.joined)
...
#------------------------------------------------------
if (writeRP){
  if (!dir.exists(rpOutputDir)){
    dir.create(rpOutputDir)
  }
rp = joined %>%
  select(subjectID, run, volume, starts_with("euclidian"), trash.joined) %>%
  mutate(trash.joined = ifelse(is.na(trash.joined), 0, trash.joined))
...
if (writePlots){
  ...
  trash.plot = trash %>%
    ...
    select(-starts_with("Diff"), -starts_with("trash"), -contains("deriv")) %>%
  ...
  nada = trash.plot %>% group_by(subjectID) %>%
    do({
      plot = ggplot(., aes(volume, value)) + 
        ...
        geom_line() + 
        geom_vline(xintercept = c(6, 33,38,65,70, 97, 102, 129)) +
